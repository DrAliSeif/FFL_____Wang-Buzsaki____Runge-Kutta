/************************************************************************************************/
/*** Topic: Wang-Buzsaki model with Euler Method for one neuron            Ali-Seif           ***/
...
//_______________Start changes________________/
    ...
    for (int i=1 ; i<=step ;i++){
        ...
        n1[i] =(n1[i-1] + dt*phi*((alpha_n_1*(1-n1[i-1]))-beta_n_1*n1[i-1]));
        h1[i] =(h1[i-1] + dt*phi*((alpha_h_1*(1-h1[i-1]))-beta_h_1*h1[i-1]));

        INa1 = g_Na*h1[i-1]*pow(m1,3)*(v1[i-1]-E_Na);
        IK1 = g_K*pow(n1[i-1],4)*(v1[i-1]-E_K);
        Il1=g_l*(v1[i-1]-E_l);

        v1[i]=(v1[i-1]+(dt)*((1/C_m)*(Iapp-(INa1+IK1+Il1))));
