...
/***        three inhibitory neurons for connect to the FFL                  Ali-Seif         ***/
...
double Isyn(double v,double s,double N) {
    int   E_s=          -75;
    float g_s=          0.1;
    float k=            1;
...
double dvdt(double t, double v,double n,double h,double s,double N) {
    float Iapp =        0.75;
    float C_m =         1.0;
    return   (1/C_m)*(Iapp -(INa(v,h)+IK(v,n)+Il(v)+Isyn(v,s,N)));
}
...
double rk4thOrder_v(double t0, double v, double dt,double n,double h,double s,double N) {
    double  k1, k2, k3, k4;
            k1=     dt*dvdt(t0, v,n,h,s,N);
            k2=     dt*dvdt((t0+dt/2), (v+k1/2),n,h,s,N);
            k3=     dt*dvdt((t0+dt/2), (v+k2/2),n,h,s,N);
            k4=     dt*dvdt((t0+dt), (v+k3),n,h,s,N);
...
int main() {
    ...
    double  t0=0,t_final=100, dt=0.01;                                          //Initial values
